<template>
  <div class="audit-page">
    ...
    <a-drawer v-model:open="drawerOpen" width="min(720px, 100vw)" title="审计事件详情" @close="closeDrawer">
      <div class="details">
        <section><h3>事件详情</h3><a-spin :spinning="detail.loading"><a-alert v-if="detail.error" type="error" message="审计详情读取失败" :description="detail.error"><template #action><a-button aria-label="重试审计详情" @click="retryDetail">重试</a-button></template></a-alert><template v-else-if="detail.data"><dl><div><dt>事件 ID</dt><dd>{{ detail.data.id }}</dd></div><div><dt>用户</dt><dd>{{ detail.data.user_id || '-' }}</dd></div><div><dt>角色</dt><dd>{{ roleLabels(detail.data.actor_roles) }}</dd></div><div><dt>错误码</dt><dd>{{ detail.data.error_code || '-' }}</dd></div></dl><p class="summary">{{ detail.data.summary || '-' }}</p><pre>{{ safeJson(detail.data.metadata) }}</pre></template></a-spin></section>
...
const categoryOptions=[{label:'全部类别',value:'all'},{label:'运行',value:'runtime'},{label:'管理',value:'management'},{label:'安全',value:'security'}];
const sourceOptions=['all','agent','tool','mcp','knowledge','sandbox','llm','system','auth'].map(value=>({label:value==='all'?'全部来源':value,value}));
...
const categoryLabels:Record<string,string>={runtime:'运行',management:'管理',security:'安全'};
const sourceLabels:Record<string,string>={agent:'Agent',tool:'工具',mcp:'MCP',knowledge:'知识库',sandbox:'沙箱',llm:'LLM',system:'系统',auth:'认证'};
...
const roleLabels=(roleCodes:string[])=>roleCodes.length?roleCodes.join(', '):'-';
