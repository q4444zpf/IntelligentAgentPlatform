...
import { agentsApi, type AgentApprovalPolicy, type AgentInfo, type AgentInput, type AgentRuntimeForm } from '@/api/agents';
...
const emptyForm = () => ({ id: '', name: '', description: '', runtime_form: 'common' as AgentRuntimeForm, language: 'zh-CN' as 'zh-CN' | 'en-US', provider_id: '', model: '', system_prompt: '', context_prompt: '', approval_policy: 'control_commands' as AgentApprovalPolicy, skill_names: [] as string[], tool_ids: [] as string[], knowledge_source_ids: [] as string[], enabled: true });
function editableInput(value: AgentInput): AgentInput { return { name: value.name, description: value.description, runtime_form: value.runtime_form, language: value.language, provider_id: value.provider_id, model: value.model, system_prompt: value.system_prompt, context_prompt: value.context_prompt, approval_policy: value.approval_policy, skill_names: [...value.skill_names], tool_ids: [...value.tool_ids], knowledge_source_ids: [...value.knowledge_source_ids], enabled: value.enabled }; }
...
function openEdit(agent: AgentInfo) { resetForm(); editingId.value = agent.id; Object.assign(form, { id: agent.id, ...editableInput(agent) }); editorTab.value = 'basic'; editorOpen.value = true; }
...
async function saveAgent() { if (!validateForm()) return; if (unavailableToolIds(form.tool_ids).length) { message.error('请先移除不可用的工具绑定再保存'); editorTab.value = 'capabilities'; return; } saving.value = true; try { const payload = editableInput(form); if (editingId.value) await agentsApi.update(editingId.value, payload); else await agentsApi.create({ id: form.id, ...payload }); message.success(editingId.value ? '智能体配置已更新' : '智能体已创建'); editorOpen.value = false; await loadAgents(); } catch (error) { message.error(error instanceof Error ? error.message : '保存失败'); } finally { saving.value = false; } }
