...
import { createIdentityProject, listIdentityProjects, setIdentityProjectStatus, type IdentityProject } from '@/api/identity';
const projects = ref<IdentityProject[]>([]); const loading = ref(false); const creating = ref(false); const createOpen = ref(false); const errorMessage = ref(''); const form = ref({ code: '', name: '' }); let controller: AbortController | null = null;
