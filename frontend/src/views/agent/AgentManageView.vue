          ...
          <a-form layout="vertical">
            ...
            <section class="tool-picker">
              ...
              <div v-else class="tool-picker-grid">
                <button v-for="tool in visibleTools" :key="tool.tool_id" type="button" class="tool-picker-row" :class="{ selected: form.tool_ids.includes(tool.tool_id), unavailable: !isToolSelectable(tool) }" :disabled="!isToolSelectable(tool)" @click="toggleToolSelection(tool)">
                  ...
                  <small v-if="!isToolSelectable(tool)">已停用，仅保留现有绑定</small>
                </button>
              </div>
...
const agents = ref<AgentInfo[]>([]); const providers = ref<ApiProvider[]>([]); const skills = ref<SkillInfo[]>([]); const tools = ref<ToolInfo[]>([]); const loading = ref(false); const loadError = ref(''); const query = ref(''); const runtimeFilter = ref('all'); const busyId = ref('');
...
async function loadAgents() { loading.value = true; loadError.value = ''; try { const [agentData, providerData, skillData, toolData] = await Promise.all([agentsApi.list(), modelProviderApi.list(), skillsApi.list(), toolsApi.list()]); agents.value = agentData; providers.value = providerData; skills.value = skillData; tools.value = toolData; } catch (error) { loadError.value = error instanceof Error ? error.message : '加载失败'; } finally { loading.value = false; } }
