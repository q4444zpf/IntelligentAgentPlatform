    ...
    <div class="conversation-layout" :class="{ 'preview-open': selectedHtmlArtifact, 'preview-medium': isMediumPreviewLayout }">
      ...
      <section class="chat-workspace" :class="{ 'mobile-active': mobilePanel === 'chat' }">
        <div class="chat-toolbar">
          <div class="mode-switch" aria-label="协作模式">
            ...
            <button data-testid="mode-team" :class="{ active: mode === 'team' }" @click="setMode('team')"><TeamOutlined /> 多智能体协同</button>
          </div>
          <div class="toolbar-selectors">
            <label class="selector-field actor-field">
              ...
              <a-select v-else v-model:value="selectedTeamId" class="actor-select" :options="teamOptions" placeholder="选择已发布团队" @change="selectTeam" />
            </label>
        ...
        <div v-if="mode === 'team' && !availableTeams.length" class="team-empty">当前项目暂无可运行团队</div>
        ...
        <div ref="message-list" class="message-stream" aria-live="polite">
          ...
          <div v-if="conversationStore.toolActivities.length" class="tool-activity" aria-label="工具活动">
            <div v-for="activity in conversationStore.toolActivities" :key="activity.invocation_id" class="tool-activity-row" :class="activity.status">
              <span class="tool-activity-state" />
              <strong>{{ activity.display_name }}</strong>
              <span>{{ activity.status === 'running' ? '执行中' : activity.status === 'completed' ? '完成' : '失败' }}</span>
              <time v-if="activity.duration_ms !== null">{{ activity.duration_ms }} ms</time>
            </div>
          </div>
          <div v-if="teamRunEvents.length" class="team-activity" aria-label="团队进度">
            <div v-for="event in teamRunEvents" :key="event.sequence"><TeamOutlined /><span>{{ teamRunEventLabel(event) }}</span></div>
          </div>
        ...
        <div class="composer-shell">
          ...
          <div class="composer-tools">
            ...
            <button data-testid="send-message" class="send-button" :disabled="!input.trim() || conversationStore.sending || (mode === 'team' && !selectedTeamId)" title="发送" @click="sendMessage"><SendOutlined /></button>
          </div>
        </div>
      </section>
      ...
      <aside v-if="!isMediumPreviewLayout" class="context-panel" :class="{ 'mobile-active': mobilePanel === 'context' }">
        ...
        <section v-if="mode === 'team'">
          <header><span>协同成员</span><b>{{ teamMembers.length }}</b></header>
      ...
      <HtmlArtifactPreview
        v-if="selectedHtmlArtifact"
        :artifact="selectedHtmlArtifact"
        @close="selectedHtmlArtifact = null"
      />
    </div>
  </main>
</template>
...
import AssistantArtifactCards from '@/components/chat/AssistantArtifactCards.vue';
import AssistantMessageContent from '@/components/chat/AssistantMessageContent.vue';
import HtmlArtifactPreview from '@/components/chat/HtmlArtifactPreview.vue';
import { isRunActive, isTeamRunEvent, runtimeStatusLabel, teamRunEventLabel } from '@/features/chat/runtimeStatus';
...
const teamVersions = ref(new Map<string, TeamVersionInfo>());
...
const teamOptions = computed(() => availableTeams.value.map((team) => ({ value: team.id, label: team.name })));
...
const activeTeam = computed(() => availableTeams.value.find((team) => team.id === selectedTeamId.value));
const activeTeamVersion = computed(() => teamVersions.value.get(selectedTeamId.value));
const activeActorName = computed(() => (mode.value === 'team' ? teamOptions.value : agentOptions.value).find((item) => item.value === (mode.value === 'team' ? selectedTeamId.value : selectedAgentId.value))?.label || '智能体');
...
const memberColors = ['#2563eb', '#0891b2', '#16a47a', '#a56818'];
const teamMembers = computed(() => {
  const definition = activeTeamVersion.value?.definition;
  if (!definition) return activeTeam.value?.supervisor ? [{ name: activeTeam.value.supervisor.agent_id, role: activeTeam.value.supervisor.responsibility, color: memberColors[0] }] : [];
  return [definition.supervisor, ...definition.members].map((member, index) => ({ name: member.agent_id, role: member.responsibility, color: memberColors[index % memberColors.length] }));
});
const teamRunEvents = computed(() => conversationStore.events.filter(isTeamRunEvent));
...
async function loadArtifactsForRuns(runSignature: string) {
  const version = ++artifactListVersion;
  artifactListController?.abort();
  artifactsByRun.value = new Map();
  if (!runSignature) return;

  const controller = new AbortController();
  artifactListController = controller;
  try {
    const artifacts = await artifactsApi.list(controller.signal);
    if (version !== artifactListVersion || controller.signal.aborted) return;
    const grouped = new Map<string, ArtifactInfo[]>();
    for (const artifact of artifacts) {
      if (artifact.run_id === null || artifact.run_id === undefined || !isHtmlArtifact(artifact)) continue;
      const runArtifacts = grouped.get(artifact.run_id) ?? [];
      runArtifacts.push(artifact);
      grouped.set(artifact.run_id, runArtifacts);
    }
...
function artifactsForRun(runId: string | null): ArtifactInfo[] {
  return runId ? artifactsByRun.value.get(runId) ?? [] : [];
}
...
async function loadTeams() {
  try {
    const teams = await teamsApi.list({ enabled: true, published: true });
    availableTeams.value = teams.filter((team) => team.enabled && team.published_version !== null);
    if (!availableTeams.value.some((team) => team.id === selectedTeamId.value)) selectedTeamId.value = availableTeams.value[0]?.id || '';
    if (selectedTeamId.value) await selectTeam(selectedTeamId.value);
  } catch {
...
async function selectTeam(teamId: string) {
  ...
  const team = availableTeams.value.find((item) => item.id === teamId);
  if (!team?.published_version || teamVersions.value.has(teamId)) return;
  try {
    const version = await teamsApi.getVersion(teamId, team.published_version);
    teamVersions.value = new Map(teamVersions.value).set(teamId, version);
  } catch {
    teamVersions.value = new Map([...teamVersions.value].filter(([id]) => id !== teamId));
  }
}
...
async function sendMessage() {
  ...
  const actorId = mode.value === 'team' ? selectedTeamId.value || undefined : selectedAgentId.value || undefined;
  if (mode.value === 'team' && !actorId) return;
...
.conversation-layout { display: grid; grid-template-columns: 250px minmax(480px, 1fr) 280px; height: calc(100vh - 166px); height: calc(100dvh - 166px); min-height: 560px; }.conversation-layout.preview-open { grid-template-columns: 250px minmax(360px, 1fr) 280px minmax(360px, 30vw); }.focus-mode .conversation-layout { height: calc(100vh - 62px); height: calc(100dvh - 62px); }.history-panel,.context-panel { min-width: 0; padding: 14px; background: #f9fcfe; }.history-panel { display: flex; flex-direction: column; border-right: 1px solid var(--line); }.context-panel { overflow-y: auto; border-left: 1px solid var(--line); }
...
.tool-activity { display: grid; max-width: min(700px, calc(100% - 43px)); gap: 4px; margin: 0 0 12px 43px; } .tool-activity-row { display: grid; grid-template-columns: 8px minmax(0, 1fr) auto auto; gap: 8px; min-width: 0; padding: 6px 10px; align-items: center; color: #607888; background: #f6f9fb; border-left: 2px solid #8ba1af; font-size: 12px; line-height: 18px; } .tool-activity-row strong { overflow: hidden; color: #315064; text-overflow: ellipsis; white-space: nowrap; } .tool-activity-row time { color: #8799a5; white-space: nowrap; } .tool-activity-state { width: 7px; height: 7px; background: #d49a25; border-radius: 50%; } .tool-activity-row.completed { border-left-color: #16a47a; } .tool-activity-row.completed .tool-activity-state { background: #16a47a; } .tool-activity-row.failed { border-left-color: #d05252; } .tool-activity-row.failed .tool-activity-state { background: #d05252; }.team-empty { padding: 9px 16px; color: #7a4b13; background: #fff8e8; border-bottom: 1px solid #f1ddb4; font-size: 12px; }.team-activity { display: grid; max-width: min(700px, calc(100% - 43px)); gap: 5px; margin: 0 0 12px 43px; }.team-activity > div { display: flex; min-width: 0; align-items: center; gap: 8px; padding: 7px 10px; color: #315064; background: #eef8f7; border-left: 2px solid #168265; font-size: 12px; }.team-activity span { overflow-wrap: anywhere; }.suggestion-row { display: flex; gap: 6px; padding: 7px 16px 0; overflow-x: auto; }.suggestion-row button { flex: none; padding: 5px 8px; color: #527085; background: #f7fafc; border: 1px solid #dce7ef; border-radius: 4px; cursor: pointer; font-size: 9px; }.composer-shell { margin: 8px 16px 14px; border: 1px solid #bdcfdd; border-radius: 6px; box-shadow: 0 3px 12px rgb(40 78 106 / 6%); }.composer-shell:focus-within { border-color: #5d8fe6; box-shadow: 0 0 0 2px rgb(37 99 235 / 9%); }.composer-shell textarea { width: calc(100% - 24px); min-height: 46px; padding: 10px 12px 3px; resize: none; color: var(--ink); background: transparent; border: 0; outline: 0; font: inherit; font-size: 11px; }.composer-tools { min-height: 34px; justify-content: space-between; padding: 0 7px 5px; }.composer-tools > div { gap: 4px; }.composer-tools button { display: grid; width: 27px; height: 27px; place-items: center; color: #708797; background: transparent; border: 0; border-radius: 4px; cursor: pointer; }.composer-tools span { margin-left: 5px; color: #9aa9b3; font-size: 8px; }.composer-tools .send-button { color: #fff; background: #2563eb; }.composer-tools .send-button:disabled { background: #a9bdd9; cursor: not-allowed; }
